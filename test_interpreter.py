...
from compiler import Compiler, BytecodeType
...
def test_simple_arithmetic(code: str, result: int):
    ...
    assert interpreter.last_value_popped == result
...
def run_computation(code: str) -> float:
    tokens = list(Tokenizer(code))
    tree = Parser(tokens).parse()
    bytecode = list(Compiler(tree).compile())
    interpreter = Interpreter(bytecode)
...
def test_arithmetic_operator_precedence(code: str, correct_precedence: str) -> None:
    assert run_computation(code) == run_computation(correct_precedence), f"Expected: {correct_precedence}"
